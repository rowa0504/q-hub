        ...
        @foreach ($all_message as $message)
            ...
            <div class="d-flex mb-3 {{ $isMe ? 'flex-row-reverse text-end' : '' }}">
                ...
                @if ($message->user->avatar)
                    <img src="{{ $message->user->avatar }}" alt="avatar" class="rounded-circle {{ $isMe ? 'ms-2' : 'me-2' }}" style="width: 40px; height: 40px; object-fit: cover;">
                @else
                    <i class="fa-solid fa-circle-user fa-2x text-secondary {{ $isMe ? 'ms-2' : 'me-2' }}"></i>
                @endif

                {{-- Message Bubble --}}
                <div class="p-2 {{ $isMe ? 'bg-primary text-white' : 'bg-white' }} border rounded" style="max-width: 75%;">
