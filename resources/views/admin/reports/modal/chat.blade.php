    ...
    <div class="modal-dialog modal-l modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content rounded-4">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="chatDetailModalLabel-{{ $report->id }}">Reported Chat Message Detail</h5>
            ...
            <div class="modal-footer bg-white d-flex justify-content-between">
                ...
                <div>
                    @if ($report->reportable->deleted_at)
                        <form action="{{ route('admin.chatMessages.activate', $report->reportable->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PATCH')
                    ...
                    @else
                        <form action="{{ route('admin.chatMessages.deactivate', $report->reportable->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
